...
compile_and_run() {
  set -e
  DEBUG_FUZZ=OFF
  cmake -DCMAKE_BUILD_TYPE=Debug -DASAN_BUILD=ON -DBUILD_TEST=ON -DDEBUG_UT=ON -DDEBUG_FUZZ=${DEBUG_FUZZ} -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -S $SRC_PATH -B $BUILD_PATH
  echo "end cmake."
  cd $BUILD_PATH
  pwd;
