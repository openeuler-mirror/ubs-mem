...
#ifndef UBSMEM_LOG_FILESINK_H
#define UBSMEM_LOG_FILESINK_H
...
class UbsmemLoggerFilesink : public UbsmemLoggerWriter {
public:
    /**
     ...
     */
    UbsmemLoggerFilesink(std::string basePath, uint32_t maxFileSize, uint32_t maxFileCount);
...
private:
    ...
    std::unordered_map<std::string, UbsmemLogFile> fileMap_{};
    std::string basePath_{};
    uint32_t maxFileSize_ = 0;
    uint32_t maxFileCount_ = 0;
    std::string auditBasePath_{};
    uint32_t auditMaxFileSize_ = 0;
    uint32_t auditMaxFileCount_ = 0;
};
} // namespace ubsmem::log

#endif // UBSMEM_LOG_FILESINK_H