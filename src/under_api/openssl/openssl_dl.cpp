...
#ifdef DEBUG_MEM_UT
    g_cryptoHandle = dlopen(ARCH_LIB_DIR "/libcrypto.so", RTLD_NOW | RTLD_LOCAL);
    g_sslHandle = dlopen(ARCH_LIB_DIR "/libssl.so", RTLD_NOW | RTLD_LOCAL);
#else
    g_cryptoHandle = dlopen(ARCH_LIB_DIR "/libcrypto.so.3", RTLD_NOW | RTLD_LOCAL);
    g_sslHandle = dlopen(ARCH_LIB_DIR "/libssl.so.3", RTLD_NOW | RTLD_LOCAL);
#endif
    if (g_cryptoHandle == nullptr || g_sslHandle == nullptr) {
